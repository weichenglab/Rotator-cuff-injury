...
rawdf.loc[(rawdf['target_y'] == 1) & (rawdf['BL2Target_yrs'] <= 10), 'target_y'] = 0
...
#### #### traditional factors + protein panel
...
search.fit(X_t_p_train, y_train)

best_model_t_p = search.best_estimator_

y_pred_proba = best_model_t_p.predict_proba(X_traditional_test)[:,1]

fpr_t_p, tpr_t_p, thresholds_t_p = roc_curve(y_train_test, y_pred_proba)
...
plt.tight_layout()
plt.savefig('13.roc_curve_over_10_comparison.pdf', dpi=300)
plt.close()
