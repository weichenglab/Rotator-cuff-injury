...
def fit_single_protein(mydf, pro_f, m1_f_lst):
    ...
    cph = CoxPHFitter()
    my_formula = "Age + Sex + Edu + target_pro"
    try:
...
results = Parallel(n_jobs=80)(
    delayed(fit_single_protein)(rawdf, pro_f, m1_f_lst) for pro_f in tqdm(pro_f_lst)
)
...
myout_df.to_csv('./02.Cox_M1_all_new.csv', index=False)
